...
If you want to know why `$?` is `255` instead of `-1`, take a look at
[this link](https://unix.stackexchange.com/questions/37915/why-do-i-get-error-255-when-returning-1).
...
The fragment above is **not legal** because some calls to `<<` are missing the left-hand operand.
...
To fix the illegal line, we just need to add `"` before `;`.
...
The `for` loop sums the numbers in the range [-100; 100]. Final value of `sum` is zero.


### Exercise 1.13

[exercise1_13_1_09.cpp](./exercise1_13_1_09.cpp)

[exercise1_13_1_10.cpp](./exercise1_13_1_10.cpp)


### Exercise 1.14

The `for` loop groups in its header everything that is needed to control the number of iterations that should be executed, and therefore it is better suited when we need to repeat a block of code a predefined number of times.

The `while` is a control statement much simpler than `for` and is normally used when the number of iterations is not known beforehand.
...
If the input values are all equal, then the program will output a single line that shows the number of occurrences of that value.

If there are no duplicated values, then the program will output one line per value and each of those lines will show that its corresponding value occurred just one time.
